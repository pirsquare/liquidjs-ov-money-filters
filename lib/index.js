...
function formatMoney(value, currency=null, options={}) {
  let withPrefix = get(options, 'withPrefix', true);
  let withTrailingZeros = get(options, 'withTrailingZeros', true);
  let native = get(options, 'native', false);

  if (!parseFloat(value) && parseFloat(value) !== 0) value = 0;

  let option = null;
  if (currency) option = CURRENCY_OPTIONS.filter((option) => option.value === currency)[0];
  if (!option) {
